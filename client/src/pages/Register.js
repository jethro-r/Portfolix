...
const handleSubmit = async (e) => {
  ...
  try {
    const userDataWithPassword = { ...userData, password }; // Include password in the object
    const response = await fetch('http://localhost:5001/api/register', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
      body: JSON.stringify(userDataWithPassword), // Send the object including password
    });
