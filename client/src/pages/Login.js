...
export default function Login() {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        axios
          .get(`http://localhost:5001/api/user/${formData.username}`)
          .then((res) => {
  ...
  const containerStyles =
    "bg-white mx-auto p-4 flex items-center justify-center h-screen";
  const inputStyles =
    "w-full p-3 border-b-4 border-navy mb-4 focus:outline-none text-xl";
        ...
        <div className="md:w-[580px]">
          ...
          <form onSubmit={handleLogin}>
            ...
            <div className="space-y-8">
              <button type="submit" className={buttonStyles1}>
                LOGIN
              </button>
            </div>
          </form>
